/*
 * lw_effect_shackle_wisdom.c -- 1:1 port of effect/EffectShackleWisdom.java
 */
#include <math.h>
#include <stddef.h>
 ...
 */
void lw_effect_shackle_wisdom_apply(LwEffect *self, struct LwState *fight) {
    (void) fight;
    ...
    if (self->value > 0) {
        lw_stats_set_stat(&self->stats, LW_STAT_WISDOM, -self->value);
        lw_entity_update_buff_stats_with(self->target, LW_STAT_WISDOM, -self->value, self->caster);
    }
}
